...
// @desc    Process auth page
// @route   POST /auth/
router.post('/', upload.single('curImg'), async (req, res) => {
    try {
        ...
        processFaceData(client, curImageKey, res)
    } catch (err) {
        console.error(err)
...
function processFaceData(client, curImageKey, res) {
    Promise.all([
        ...
        const labeledFaceDescriptors = await loadLabeledImages(client)
        const faceMatcher = new faceapi.FaceMatcher(labeledFaceDescriptors, 0.6)
        const image = await canvas.loadImage(`http://localhost:5000/clients/images/${curImageKey}`)
...
async function loadLabeledImages(client) {
    const descriptions = []
    try {
        let img = await canvas.loadImage(`http://localhost:5000/clients/images/${client.image1Key}`)
        ...
        img = await canvas.loadImage(`http://localhost:5000/clients/images/${client.image2Key}`)
